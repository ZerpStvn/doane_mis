...
switch($action){
	case "process": // case process insert the form data in DB and process to the paypal
		mysql_query("INSERT INTO `purchases` (`invoice`, `product_id`, `product_name`, `product_quantity`, `product_amount`, `payer_fname`, `payer_lname`, `payer_address`, `payer_city`, `payer_state`, `payer_zip`, `payer_country`, `payer_email`, `payment_status`, `posted_date`) VALUES ('".$_POST["invoice"]."', '".$_POST["product_id"]."', '".MJ_cmgt_strip_tags_and_stripslashes($_POST["product_name"])."', '".MJ_cmgt_strip_tags_and_stripslashes($_POST["product_quantity"])."', '".MJ_cmgt_strip_tags_and_stripslashes($_POST["product_amount"])."', '".MJ_cmgt_strip_tags_and_stripslashes($_POST["payer_fname"])."', '".MJ_cmgt_strip_tags_and_stripslashes($_POST["payer_lname"])."', '".MJ_cmgt_strip_tags_and_stripslashes($_POST["payer_address"])."', '".MJ_cmgt_strip_tags_and_stripslashes($_POST["payer_city"])."', '".MJ_cmgt_strip_tags_and_stripslashes($_POST["payer_state"])."', '".$_POST["payer_zip"]."', '".MJ_cmgt_strip_tags_and_stripslashes($_POST["payer_country"])."', '".$_POST["payer_email"]."', 'pending', NOW())");
		...
		$p->add_field('item_name_1', MJ_cmgt_strip_tags_and_stripslashes($_POST["product_name"]));
		$p->add_field('item_number_1', $_POST["product_id"]);
		$p->add_field('quantity_1', MJ_cmgt_strip_tags_and_stripslashes($_POST["product_quantity"]));
		$p->add_field('amount_1', $_POST["product_amount"]);
		$p->add_field('first_name', MJ_cmgt_strip_tags_and_stripslashes($_POST["payer_fname"]));
		$p->add_field('last_name', MJ_cmgt_strip_tags_and_stripslashes($_POST["payer_lname"]));
		$p->add_field('address1', MJ_cmgt_strip_tags_and_stripslashes($_POST["payer_address"]));
		$p->add_field('city', MJ_cmgt_strip_tags_and_stripslashes($_POST["payer_city"]));
		$p->add_field('state', MJ_cmgt_strip_tags_and_stripslashes($_POST["payer_state"]));
		$p->add_field('country', MJ_cmgt_strip_tags_and_stripslashes($_POST["payer_country"]));
