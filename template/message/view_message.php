...
if($_REQUEST['from']=='sendbox')
{
	...
	MJ_cmgt_change_read_status_reply(sanitize_text_field($_REQUEST['id']));
	$author = $message->post_author;	
	if(isset($_REQUEST['delete']))
	{
		wp_delete_post(sanitize_text_field($_REQUEST['id']));
		$obj_message->MJ_cmgt_delete_message(sanitize_text_field($_REQUEST['id']));
		wp_safe_redirect(home_url()."?church-dashboard=user&&page=message&tab=sentbox" );
		exit();
	}
...
{
	$message = $obj_message->MJ_cmgt_get_message_by_id(sanitize_text_field($_REQUEST['id']));
	$message1 = get_post(sanitize_text_field($message->post_id));
	$author = sanitize_text_field($message1->post_author);
	MJ_cmgt_change_read_status(sanitize_text_field($_REQUEST['id']));
	MJ_cmgt_change_read_status_reply(sanitize_text_field($message->post_id));
	$box='inbox';
	if(isset($_REQUEST['delete']))
	{
		wp_delete_post(sanitize_text_field($message->post_id));
		$obj_message->MJ_cmgt_delete_message(sanitize_text_field($message->post_id));
		//$obj_message->MJ_cmgt_delete_message($_REQUEST['id']);
		wp_safe_redirect(home_url()."?church-dashboard=user&page=message&tab=inbox&message=2" );
		exit();
	}
}
//DELETE  MESSAGE DATA
...
if(isset($_POST['replay_message']))
{
	...
	if($result)
		wp_safe_redirect(home_url()."?church-dashboard=user&&page=message&tab=view_message&from=".$message_from."&id=$message_id&message=1" );
}
...
if(isset($_REQUEST['action'])&& $_REQUEST['action']=='delete-reply')
{
	...
	{
		$result=$obj_message->MJ_cmgt_delete_reply(sanitize_text_field($_REQUEST['reply_id']));
		if($result)
		{
			wp_redirect ( home_url().'?church-dashboard=user&page=message&tab=view_message&action=delete-reply&from='.$message_from.'&id='.$message_id.'&message=2');
		}
	}
}
?>
	<div class="mailbox-content"><!-- MAILBOX CONTENT DIV START -->
		<div class="message-header">
			<h3><span><?php _e('Subject','church_mgt')?> :</span>  <?php if($box=='sendbox'){ echo $message->post_title; 
			} else{ echo $message->msg_subject; } ?></h3>
			<p class="message-date"><?php  if($box=='sendbox') echo  mysql2date('d/m/y', $message->date ); else echo  mysql2date('d/m/y', esc_attr($message->msg_date) ) ;?></p>
		</div>
		<div class="message-sender">                                
			<p><?php if($box=='sendbox'){
					$message_for=get_post_meta($_REQUEST['id'],'message_for',true);
					
					echo esc_html_e('From:','church_mgt')." ".MJ_cmgt_church_get_display_name($message->post_author)."<span>&lt;".MJ_cmgt_get_emailid_byuser_id(esc_attr($message->post_author))."&gt;</span><br>";
					
					if($message_for == 'user'){
					echo __('To','church_mgt').MJ_cmgt_church_get_display_name(get_post_meta($_REQUEST['id'],'message_for_userid',true))."<span>&lt;".MJ_cmgt_get_emailid_byuser_id(get_post_meta($_REQUEST['id'],'message_for_userid',true))."&gt;</span><br>";
					}
					else{
						echo __('To','church_mgt')." ".__('Group','church_mgt');
					}
				 } else{ 
					echo esc_html_e('From :','church_mgt')."".MJ_cmgt_church_get_display_name($message->sender)."<span>&lt;".MJ_cmgt_get_emailid_byuser_id(esc_attr($message->sender))."&gt;</span><br> ".__('To','church_mgt').": ".MJ_cmgt_church_get_display_name($message->receiver);  ?> <span>&lt;<?php echo MJ_cmgt_get_emailid_byuser_id(esc_attr($message->receiver));?>&gt;</span>
		...
		<div class="message-content"><!-- MAILBOX CONTENT DIV START -->
			<p><?php $receiver_id=0;
			if($box=='sendbox'){ echo $message->post_content; $receiver_id=(get_post_meta($_REQUEST['id'],'message_for_userid',true)); 
			}else{
			...
			if($user_access['delete'] == '1') {
				...
				<div class="message-options pull-right">
					<a class="btn delete_btn_css" href="?church-dashboard=user&&page=message&tab=view_message&id=<?php echo $_REQUEST['id'];?>&from=<?php echo $box;?>&delete=1" onclick="return confirm('<?php _e('Are you sure you want to delete this record?','church_mgt');?>');"><i class="fa fa-trash m-r-xs"></i><?php _e('Delete','church_mgt')?></a> 
				</div>
			...
			{ 
				if($reply->sender_id == get_current_user_id() || $reply->receiver_id == get_current_user_id())
				{
					...
					<div class="message-content">
						...
						<a href="?church-dashboard=user&page=message&tab=view_message&action=delete-reply&from=<?php echo $_REQUEST['from'];?>&id=<?php echo $_REQUEST['id']; ?>&reply_id=<?php echo $reply->id; ?>"><?php _e('Delete','church_mgt');?></a></span> 
						<?php }
			...
			<div class="message-content float_left_width_100 replay_message_div padding_top_25px">
				<div class="form-body user_form mt-2">
										...
										<select name="receiver_id[]" class="form-control" id="selected_users" multiple="true">
											...
											{  	
												...
												{								
													...
													{
														?>
														<option  value="<?php echo esc_attr($reply_to_user->receiver);?>" ><?php echo MJ_cmgt_church_get_display_name(esc_attr($reply_to_user->receiver)); ?></option>
														<?php
													}
												}							
											} 
			 	...
			 	<!-- <div class="col-sm-12">
					<div class="form-group" >
						<div class="mb-3 row">	
							<label class="col-sm-3 control-label" ><?php _e('Select user to reply','church_mgt');?><span class="require-field">*</span></label>
				...
				<div class="col-sm-12">
					<div class="form-group">
						<div class="mb-3 row">	
							<label class="col-sm-3 control-label" for="photo"><?php _e('Message Comment','church_mgt');?><span class="require-field">*</span></label>
				...
				<div class="message-options pull-right reply-message-btn">
					<button type="submit" name="replay_message" class="btn btn-success" id="check_reply_user"><?php _e('Send','church_mgt')?></button>
				</div> -->
